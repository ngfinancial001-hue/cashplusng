...
const createVirtualAccount = async ({ accountReference, accountName, currencyCode = 'NGN', contractCode, customerEmail, customerName }) => {
  const client = await monnifyAxios();
...
const verifyTransaction = async (transactionReference) => {
  const client = await monnifyAxios();
  const response = await client.get(`/api/v2/transactions/${encodeURIComponent(transactionReference)}`);
  return response.data;
};
...
module.exports = { createVirtualAccount, verifyTransaction, verifyMonnifyWebhook, getMonnifyToken };
